...
use crate::{view::{components::{S3List, FileCRUD, SelectableContainer, ListEntry, State, StatefulContainer, FilesystemList, FileList, FileEntry}, utils}, aws::s3::S3Object};
use crate::aws::{Kind};
...
pub struct MainScreen<'clilife> {
    ...
    s3_list: Box<dyn FileList + 'clilife>,
    fs_list: Box<dyn FileList + 'clilife>,
}
...
impl<'clilife> MainScreen<'clilife> {
    ...
    pub fn get_filesystem_list(path: &Path) -> utils::StatefulList {
        utils::StatefulList::with_items(
            read_dir(path)
                .unwrap()
                .map(|f| {
    ...
    pub async fn handle_event(&mut self, event: KeyEvent) {
        let mut curr_list = self.get_curr_list();
    ...
    fn get_curr_list(&mut self) -> &mut Box<dyn FileList + 'clilife> {
    ...
    pub fn render(&mut self) -> Result<(), Box<dyn Error>> {
        self.term.draw(|f| {
            ...
            //     "{}/{}",
            //     self.s3_list.get_bucket_name(),
            //     "S3"
            // );
            let s3_pane_name = "S3Bucket";
...
fn make_file_list<'a>(name: &'a str, items: &'a [ListEntry<FileEntry>], is_focused: bool) -> List<'a> {
...
fn transform_list(options: &[ListEntry<FileEntry>]) -> Vec<ListItem> {
    options
        .iter()
        .map(|o| {
